...
def predict(args, model, tokenizer):
    ...
    # based on the particular tagging scheme, cast all possible tags into possible values in a dict
    ...
    # run for all datasets, using tqdm to visualise training
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # base prediction line (pretrained?)
            ...
            for t in p_ts_sequence:
                ...
                output_json[aspect[0]] = sentiment
            data = {
               "input": ' '.join(words),
               "output": output_json
             }
        ...
        idx += 1
    output['count'] = idx
    print(f"No. Examples: {idx}, Positive count: {pos_count}, Negative count: {neg_count}, Neutral count: {neu_count}"  )
    # file.close()
    jsonString = json.dumps(output)
    jsonFile = open(args.data_dir + "/output.json", "w")
